...
export const InstructionList = ({ recipeInstructions, setRecipeInstructions }) => {
  const [newInstruction, setNewInstruction] = useState("");
  const [heights, setHeights] = useState(Array(recipeInstructions.length).fill("auto"));
  ...
  const handleTextAreaHeightChange = (event, index) => {
    ...
    style.height = "auto";
    style.height = `${Math.max(textareaLineHeight, event.target.scrollHeight)}px`;
  ...
  const handleAddInstruction = () => {
    if (recipeInstructions.some((instruction) => instruction === "")) {
      alert("Please fill in all previous fields before adding a new Instruction.");
      return;
    }
